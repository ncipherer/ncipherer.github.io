...
export class NotePage extends AbstractView {
  ...
  private async renderMovies(content: string): Promise<string> {
    ...
    setTimeout(() => {
      ...
      const openReview = (modal: HTMLElement) => {
        modal.classList.add("active");
        document.body.style.overflow = "hidden";
      };
      const closeReview = (modal: HTMLElement) => {
        modal.classList.remove("active");
        document.body.style.overflow = "";
      };
      const activeReview = () =>
      ...
      document.querySelectorAll(".movie-card.has-review").forEach((card) => {
        card.addEventListener("click", (e) => {
          ...
          if (movieId) {
            ...
            if (modal) {
              openReview(modal as HTMLElement);
            }
          }
        });
      });
      ...
      reviewModals.forEach((modal, index) => {
        const closeModal = () => closeReview(modal);
  ...
  private async renderBooks(content: string): Promise<string> {
    ...
    const spinesHtml = allBooks.map((book: any, i: number) => {
      ...
      const widthRem = (6.4 + (book.title.length % 4) * 0.28).toFixed(2);
      const slant = ((i - (allBooks.length - 1) / 2) * 1.8).toFixed(1);
      ...
      return `<div class="shelf-book" data-book-index="${i}" data-cover="${cover}" style="--book-hue:${hue};--book-width:${widthRem}rem;--slant:${slant}deg;--i:${i}" title="${book.title.replace(/"/g, '&quot;')}">
  <div class="shelf-book-cover"><span class="shelf-book-label">${book.title}</span>${readBadge}</div>
</div>`;
    }).join("\n");

    const booksJson = JSON.stringify(
      allBooks.map((b: any) => ({ ...b, cover: coverFor(b), hue: hashHue(b.title) })),
    ).replace(/</g, "\\u003c");
...
<script type="application/json" id="shelf-books-data">${booksJson}</script>`;
  ...
  private setupBookshelfInteractivity(): void {
    ...
    booksData.forEach((book, idx) => {
      ...
      fetch(`https://openlibrary.org/search.json?q=intitle:${encodeURIComponent(book.title)}`)
        ...
        .then((data: any) => {
          ...
          book.cover = cover;
          // Persist the lookup so future visits skip this search entirely.
          safeSetItem(`book_cover_${book.title}`, cover);
          const el = track.querySelector(`.shelf-book[data-book-index="${idx}"]`) as HTMLElement | null;
          if (el) {
    ...
    // Show a book in the modal and highlight it on the shelf
    const openBook = (idx: number) => {
      const book = booksData[idx];
      if (!book) return;
      const bookEl = track.querySelector<HTMLElement>(`.shelf-book[data-book-index="${idx}"]`);
      if (activeBook) activeBook.classList.remove("shelf-book--active");
      activeBook = bookEl;
      if (bookEl) bookEl.classList.add("shelf-book--active");
      ...
      // Keep the active book in view on the shelf (useful when skimming)
      if (bookEl) bookEl.scrollIntoView({ behavior: "smooth", inline: "center", block: "nearest" });
    };
    ...
    const navigateBook = (dir: 1 | -1): boolean => {
      ...
      const currentIdx = activeBook
        ? parseInt(activeBook.dataset.bookIndex || "0", 10)
        : 0;
