...
import { CacheRevalidator } from "./services/cache";
import { SonarUIController } from "./services/sonar-ui";
...
class App {
  ...
  constructor() {
    ...
    // Sonar scroll is a site-wide setting (like theme/sound): navbar toggle,
    // persisted across visits, works on every page.
    SonarUIController.getInstance().init();
    // Time is counted on every route, not just the homepage — the garden that
    // the homepage shows is fed by the whole site.
    GardenService.getInstance().init();
  }
  ...
  init() {
    ...
    document.addEventListener("DOMContentLoaded", async () => {
      ...
      // Serve every cached lookup from localStorage, then revalidate the
      // whole cache in the background with high jitter — silent updates that
      // the next page load picks up.
      NoteService.getInstance().registerCacheRevalidation();
      CacheRevalidator.getInstance().start();
