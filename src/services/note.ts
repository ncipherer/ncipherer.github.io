...
/**
 * Drafts that are intentionally not shipped to /data (see the CopyWebpackPlugin
 * ignore list in webpack.config.js). Fetching them would 404 on every page
 * load, so the prefetchers skip them and the note page shows a friendly
 * "coming soon" body instead.
 */
const PRIVATE_NOTE_PATHS = new Set(["i-just-wanna-see.md"]);
...
export class NoteService {
  ...
  public async fetchNoteContent(note: Note): Promise<string> {
    ...
    if (note.contentPath && PRIVATE_NOTE_PATHS.has(note.contentPath)) {
      // Unshipped draft: return a placeholder without touching the network,
      // so neither the page render nor the prefetcher emits a 404.
      note.content = "*This entry is still a work in progress — check back soon.*";
      return note.content;
    }
    try {
  ...
  /**
   * Fetch a movie's details from OMDB (with a TMDB fallback) — the single
   * source used both when warming the cache and when revalidating it.
   */
  private async resolveMovieDetails(movie: {
    title: string;
    year?: string;
    imdbID?: string;
    tmdbID?: string;
    ...
    const omdbUrl = movie.imdbID
      ...
      : `https://www.omdbapi.com/?t=${encodeURIComponent(movie.title)}&y=${movie.year}&apikey=${process.env.OMDB_API_KEY}`;
    ...
    try {
      ...
      // Fallback to TMDB because OMDB failed or has no poster.
  ...
  /** Look up a non-ISBN book's cover URL from the Open Library search API. */
  private async resolveBookCoverUrl(title: string): Promise<string | null> {
    const res = await fetch(
      `https://openlibrary.org/search.json?q=intitle:${encodeURIComponent(title)}`,
    );
    if (!res.ok) return null;
    const data = await res.json();
    const info = data && data.docs && data.docs[0];
    return info && info.cover_i
  ...
  private async warmMovieCover(movie: {
    ...
    const entry = await this.resolveMovieDetails(movie);
  ...
  private async warmBookCover(book: { title: string; isbn: string | null }): Promise<void> {
    // Book covers are loaded with CORS so their bytes can be persisted as
    // data URLs — a later visit then renders them straight from localStorage.
    if (book.isbn) {
      const url = `https://covers.openlibrary.org/b/isbn/${book.isbn}-M.jpg`;
      const img = await this.preloadImage(url, true);
      if (img) persistCoverImage(url, img);
      return;
    }
    const cacheKey = `book_cover_${book.title}`;
    const cached = localStorage.getItem(cacheKey);
    if (cached) {
      const img = await this.preloadImage(cached, true);
      if (img) persistCoverImage(cached, img);
      return;
    }
    await jitteredDelay(150, 500);
    const url = await this.resolveBookCoverUrl(book.title);
    if (url) {
  ...
  /**
   * Background revalidation: re-run the network call behind each cached
   * entry and refresh the stored value (and the live page, via events) when
   * the source changed. Transient failures keep the existing cache.
   */
  private async revalidateMovieEntry(cacheKey: string): Promise<void> {
    ...
    const imdbID = cacheKey.startsWith("movie_imdb_")
      ? cacheKey.slice("movie_imdb_".length)
      : undefined;
    const fresh = await this.resolveMovieDetails({
      title: old.title || "",
      year: old.year,
      imdbID,
    });
    if (!fresh || !fresh.poster) return; // lookup failed — keep existing cache
    const changed =
      fresh.title !== old.title ||
      fresh.year !== old.year ||
      fresh.director !== old.director ||
      fresh.poster !== old.poster;
    if (!changed) return;
    // The cache is refreshed silently; the page picks it up on next load.
    localStorage.setItem(cacheKey, JSON.stringify(fresh));
  }

  private async revalidateBookCoverUrl(title: string): Promise<void> {
    const cacheKey = `book_cover_${title}`;
    const url = await this.resolveBookCoverUrl(title);
    if (!url) return; // lookup failed — keep existing cache
  ...
  private async revalidateCoverImage(url: string): Promise<void> {
    const key = coverImageCacheKey(url);
    const cached = getCachedCoverImage(url);
    try {
      const res = await fetch(url);
      if (!res.ok) {
        // The cover no longer exists — drop the cached bytes so the plain
        // colour fallback shows instead of a broken image.
        localStorage.removeItem(key);
        return;
      }
      const blob = await res.blob();
      const fresh = await blobToDataUrl(blob);
      if (cached && fresh === cached.data) return; // still correct
      let w = cached ? cached.w : 0;
      let h = cached ? cached.h : 0;
      try {
        ...
        // keep the previous dimensions
      }
      const entry = { data: fresh, w, h };
      // The cache is refreshed silently; the page picks it up on next load.
      localStorage.setItem(key, JSON.stringify(entry));
    } catch {
      // transient network failure — keep the cached bytes
    }
  }

  /**
   * Register a revalidation task for every entry currently in the caches
   * (movie details, book cover lookups, cached cover images). Call once at
   * app start, before CacheRevalidator.start().
   */
  public registerCacheRevalidation(): void {
    const revalidator = CacheRevalidator.getInstance();
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (!key) continue;
      if (key.startsWith("movie_imdb_") || key.startsWith("movie_title_")) {
        revalidator.register(key, () => this.revalidateMovieEntry(key));
      } else if (key.startsWith("book_cover_img_")) {
        const url = key.slice("book_cover_img_".length);
        revalidator.register(key, () => this.revalidateCoverImage(url));
      } else if (key.startsWith("book_cover_")) {
        const title = key.slice("book_cover_".length);
        revalidator.register(key, () => this.revalidateBookCoverUrl(title));
      }
    }
  }

  private preloadImage(url: string, cors = false): Promise<HTMLImageElement | null> {
    return new Promise((resolve) => {
      const img = new Image();
      // cors=true loads into the CORS cache partition, which is what
      // applyCover() reuses for persistence on the books page. Movie posters
      // stay plain so the browser cache keeps serving the <img> tags.
        ...
        const movieCards = await Promise.all(
          section.movies.map(async (movie: any) => {
            ...
            try {
              let url = "";
              if (movie.imdbID) {
                url = `https://www.omdbapi.com/?i=${movie.imdbID}&apikey=${process.env.OMDB_API_KEY}`;
              } else {
                url = `https://www.omdbapi.com/?t=${encodeURIComponent(movie.title)}&y=${movie.year}&apikey=${process.env.OMDB_API_KEY}`;
              }
